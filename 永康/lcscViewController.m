...
@interface lcscViewController ()<UITableViewDelegate,UITableViewDataSource>
{
...
@implementation lcscViewController
...
-(void)json
{
    ...
    NSURLRequest *request=[NSURLRequest requestWithURL:[NSURL URLWithString:@"http://114.215.170.1:8004/qsxk/api/GSService/getLcscList"]];
    ...
    if (response) {
        ...
        for (int i=0; i<arrNew.count; i++) {
            ...
            [arrSywz addObject:[dicNew objectForKey:@"sywz"]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *dicPass=@{@"scid":[arrScid objectAtIndex:indexPath.row],@"type":@"lcsc",@"name":[arrScmc objectAtIndex:indexPath.row]};
